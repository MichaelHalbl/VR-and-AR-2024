...
public class StartWire : MonoBehaviour
{
    ...
    void Update()
    {
        // Überprüfen, ob der Spieler in Reichweite ist und die A-Taste drückt, und ob die Szene nicht bereits geladen wird
        if (isInRange && !isLoading && rightHandController.IsPressed(InputHelpers.Button.PrimaryButton, out bool isPressedA, 0.1f) && isPressedA)
        {
            ...
            StartCoroutine(LoadSceneAsync("WireGame"));
        }
    }
